...
const KrijoHuazimin: React.FC<IProps> = ({
  ...
  const handleSubmit = () => {
    ...
    let newHuazimi: IHuazimi = {
        huazimiId: 0,
        currentDate: currentDate.toString(), // Convert to string
        dueDate: dueDate.toString(), // Convert to string
        returnDate: new Date(),
        isReturned: false,
        isbn: selectedLibri.isbn, // Use the selected libri's isbn
        id: "not from rezervim", // Use the selected user's id
  ...
  return (
    ...
    <Dialog header="Krijo Huazimin" visible={show} style={{ width: '30vw' }} onHide={onHide}>
      ...
      <Dropdown value={selectedLibri} onChange={(e) => setSelectedLibri(e.value)} options={librat.map((libri) => ({
        label: libri.titulli + " " + libri.isbn,
        value: libri,
        key: libri.isbn
      }))} optionLabel="label" 
